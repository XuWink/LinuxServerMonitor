...
bool RpcClient::SetMonitorInfo(const monitor::proto::MonitorInfo & monitor_info) {
    // 用于管理 gRPC 调用的生命周期
...
bool RpcClient::GetAllMonitorInfo(AllMonitorInfo * all_monitor_info) {
    ...
    // 调用 RPC 方法
    ::grpc::Status status = stub_ptr_->GetAllMonitorInfo(&context, request, all_monitor_info);
