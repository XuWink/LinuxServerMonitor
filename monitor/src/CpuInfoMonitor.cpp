...
void CpuInfoMonitor::UpdateOnce(monitor::proto::MonitorInfo * monitor_info) {
    ...
    // 一次性读取整个文件内容到字符串
    std::string file_content((std::istreambuf_iterator<char>(cpuinfo_file)),  // 开始迭代器
...
/// @param monitor_info
monitor::CpuInfoMonitor::CpuInfo CpuInfoMonitor::ParseCpuSection(const std::string & section) {
    CpuInfo            cpu_info;
    ...
    while (std::getline(section_stream, line)) {
        ...
        if (key == "processor") {
            ...
            numeric_value.erase(std::remove_if(numeric_value.begin(), numeric_value.end(), ::isalpha),
                                numeric_value.end());
            try {
